...
def classify_image(img_path, threshold=0.7):
    ...
    if confidence < threshold:
        return "Nem ismert tábla", confidence
    else:
        return classes[predicted_class], confidence

# Fájl betöltés és megjelenítés
...
def upload_image():
    ...
    # Osztályozás
    label, conf = classify_image(file_path)
    result_text.set(f"Eredmény: {label}\nBizonyosság: {conf:.2%}")
...
btn = tk.Button(frame, text="Kép kiválasztása", command=upload_image)
btn.pack()
...
result_label = tk.Label(frame, textvariable=result_text, font=(
    "Arial", 12), wraplength=300, justify="center")
